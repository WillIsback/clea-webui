...
export async function listDocuments(
	...
	let url = `${getApiUrl()}/database/documents?skip=${skip}&limit=${limit}`;
...
export async function getDocument(id: number): Promise<DocumentOut> {
	const response = await fetch(`${getApiUrl()}/database/documents/${id}`);
	if (!response.ok) {
...
export async function addDocument(
	...
	const res = await fetch(`${getApiUrl()}/database/documents`, {
		method: 'POST',
...
export async function updateDocument(document: DocumentUpdate): Promise<Document> {
	const response = await fetch(`${getApiUrl()}/database/documents/${document.id}`, {
		method: 'PUT',
		headers: { 'Content-Type': 'application/json' },
...
export async function updateDocumentWithChunks(payload: UpdateWithChunks): Promise<Document> {
	const response = await fetch(`${getApiUrl()}/database/documents/${payload.document.id}`, {
		method: 'PUT',
		headers: { 'Content-Type': 'application/json' },
...
export async function deleteDocument(documentId: number): Promise<{ success: boolean }> {
	const response = await fetch(`${getApiUrl()}/database/documents/${documentId}`, {
		method: 'DELETE'
	});
	if (!response.ok) {
...
export async function getDocumentChunks(
	...
	let url = `${getApiUrl()}/database/documents/${documentId}/chunks`;
	if (hierarchyLevel !== undefined) url += `?hierarchyLevel=${hierarchyLevel}`;
	if (parentChunkId !== undefined)
		url += `${hierarchyLevel !== undefined ? '&' : '?'}parentChunkId=${parentChunkId}`;
