<script lang="ts">
    ...
    import type { AskAIRequest } from '$lib/types';
    ...
    let selectedModel = $state('Qwen/Qwen3-0.6B');
    ...
    let activeContext = $state<any>(null);
    ...
    function openContext(context: any) {
    ...
    async function sendMessage() {
        ...
        try {
            if (useStreaming) {
                        ...
                        if (line.startsWith('data: ')) {
                            ...
                            try {
                                ...
                                if (jsonData.type === 'thinking') {
                                    ...
                                    // Transition entre thinking et response
                ...
                const lastIndex = messages.length - 1;
                messages[lastIndex].context = context;
            ...
            error = err instanceof Error ? err.message : 'Une erreur est survenue lors de la communication avec l\'API';
...
<main class="p-4 max-w-6xl mx-auto">
        ...
        <div class="bg-base-200 p-4 rounded-lg mb-6 shadow-md">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div class="form-control">
                    ...
                    <div class="flex flex-col gap-2">
                        <label class="label cursor-pointer justify-start gap-2">
                            <input type="checkbox" class="toggle toggle-primary toggle-sm" bind:checked={useStreaming} />
                            <span class="label-text">Réponse en streaming</span>
                        </label>
                        <label class="label cursor-pointer justify-start gap-2">
                            <input type="checkbox" class="toggle toggle-primary toggle-sm" bind:checked={enableThinking} />
                ...
                <div class="form-control">
                    ...
                    <input id="theme" type="text" class="input input-bordered w-full" bind:value={theme} placeholder="Filtrer par thème" />
                </div>
                ...
                <div class="form-control">
                    ...
                    <input id="documenType" type="text" class="input input-bordered w-full" bind:value={documentType} placeholder="Filtrer par type" />
                </div>
                ...
                <div class="form-control">
                    ...
                    <input id="corpusId" type="text" class="input input-bordered w-full" bind:value={corpusId} placeholder="ID du corpus (optionnel)" />
                </div>
                ...
                <div class="form-control">
                    ...
                    <input id="date-start" type="date" class="input input-bordered w-full" bind:value={startDate} />
                </div>
                ...
                <div class="form-control">
                    ...
                    <input id="data-end" type="date" class="input input-bordered w-full" bind:value={endDate} />
                </div>

                <!-- Options hiérarchiques -->
                <div class="form-control">
                    <label class="label cursor-pointer justify-start gap-2">
                        <input type="checkbox" class="toggle toggle-primary toggle-sm" bind:checked={useHierarchical} />
    ...
    <div class="flex flex-col h-[60vh] border rounded-lg overflow-hidden shadow-lg">
        <!-- Zone des messages -->
        <div 
            class="flex-grow overflow-y-auto p-4 bg-base-100"
        ...
        {#each messages as message, i (i)}
            <div class="chat {message.role === 'user' ? 'chat-end' : 'chat-start'} mb-4">
                ...
                <div class="chat-bubble {message.role === 'user' ? 'chat-bubble-accent' : 'chat-bubble-primary'} whitespace-pre-wrap break-words">
                    {#if message.thinking && enableThinking}
                        <div class="thinking-content">
                            <details>
                                <summary class="cursor-pointer text-xs">Voir le raisonnement</summary>
                                <div class="text-xs opacity-75 italic mt-1 mb-2 border-l-2 pl-2 border-base-content/30">
                                    {message.thinking}
                                </div>
                            </details>
                        </div>
        ...
        <div class="p-4 bg-base-200 border-t">
            ...
            <p class="text-xs text-base-content/70 mt-2">
                Utilisez cette interface pour poser des questions sur les documents de votre base de connaissances.
            </p>
        </div>
    </div>
</main>
        ...
        <div class="p-4 w-full max-w-xl h-full bg-base-100 overflow-y-auto flex flex-col">
                ...
                <div class="flex flex-col gap-2 flex-grow">
                    ...
                    <div class="flex flex-col gap-1 p-3 bg-base-200 rounded-lg">
                        ...
                        {#if activeContext.confidence}
                            <p>
                                ...
                                <span class="{activeContext.confidence.level > 0.7 ? 'text-success' : activeContext.confidence.level > 0.4 ? 'text-warning' : 'text-error'}">
                    ...
                    {#if activeContext.results && activeContext.results.length > 0}
                        <div class="overflow-y-auto flex-grow">
                            {#each activeContext.results as result, index}
                                <div class="border rounded p-3 mb-2 {index === 0 ? 'border-accent bg-accent/5' : ''}">
                                    <div class="flex justify-between items-center">
                                        <h5 class="font-semibold">{result.title}</h5>
                                        <div class="badge {activeContext.normalized ? 
                                            (result.score > 0.8 ? 'badge-success' : result.score > 0.5 ? 'badge-warning' : 'badge-error') : 
                                            'badge-primary'}">
                                            Score: {activeContext.normalized ? `${(result.score * 100).toFixed(1)}%` : result.score.toFixed(3)}
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex gap-2 mt-2 flex-wrap text-xs">
                                        ...
                                        {#if result.publishDate}
                                            <div class="badge badge-outline">{new Date(result.publishDate).toLocaleDateString()}</div>
                                        {/if}
            ...
            {:else}
                <div class="alert alert-info">
                    Aucun contexte disponible pour cette réponse.
                </div>
            {/if}
        </div>
    </div>
