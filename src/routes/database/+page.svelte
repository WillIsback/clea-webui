<script lang="ts">
	...
	async function cleanupIndexes() {
		...
		try {
			...
			console.error(cleanupMessage, err);
		} finally {
			isCleaning = false;
	...
	async function deleteSelectedDocuments() {
		...
		try {
			...
			deleteError =
				err instanceof Error ? err.message : 'Une erreur lors de la suppression des documents.';
			deleteSuccess = '';
		} finally {
			isDeleting = false;
		}
...
<main class="p-4 max-w-7xl mx-auto">
	...
	<div class="flex flex-wrap items-center justify-between gap-4 mb-4 border-b pb-4">
		...
		{#if selectedCount > 0}
			<button class="btn btn-sm btn-error" on:click={deleteSelectedDocuments} disabled={isDeleting}>
				{#if isDeleting}
					<span class="loading loading-spinner loading-xs"></span>
				{:else}
					<svg
