<script lang="ts">
	...
	import type { DocumentOut, ChunkCreate, IndexStatus, Document } from '$lib/types';
	import { onMount } from 'svelte';
	import { goto } from '$app/navigation';

	// ID du document à consulter depuis l'URL
	const documentId = page.url.searchParams.get('id')
		? parseInt(page.url.searchParams.get('id') || '0')
		: null;

	// Récupérer l'ID du chunk à mettre en évidence depuis l'URL
	const highlightChunkId = page.url.searchParams.get('highlight')
		? parseInt(page.url.searchParams.get('highlight') || '0')
		: null;
	...
	let editedDocument = $state<Document | null>(null);
		...
		try {
			...
			if (document.corpusId) {
				try {
					...
					indexStatus = {
						corpusId: apiIndexStatus.corpusId,
						indexExists: apiIndexStatus.indexExists,
						configExists: apiIndexStatus.configExists,
						isIndexed: apiIndexStatus.isIndexed,
						indexType: apiIndexStatus.indexType || null,
						chunkCount: apiIndexStatus.chunkCount,
						indexedChunks: apiIndexStatus.indexedChunks,
						lastIndexed: apiIndexStatus.lastIndexed
					};
				} catch (err) {
	...
	async function handleUpdateDocument() {
		...
		try {
			const result = await updateDocument(editedDocument);
	...
	async function handleDeleteChunk(chunkId: number) {
		...
		try {
			...
			updateError =
				err instanceof Error ? err.message : 'Une erreur lors de la suppression du chunk.';
			updateSuccess = '';
		}
	}
	...
	async function handleCreateIndex() {
		...
		try {
			...
			indexError =
				err instanceof Error ? err.message : "Une erreur lors de la création de l'index.";
		} finally {
			indexing = false;
		}
	}
	...
	function startEditing() {
		...
		editedDocument = {
			...
			indexNeeded: document.indexNeeded || false
		};
	...
	function searchChunks() {
		...
		switch (chunkSearchAttribute) {
			case 'content':
				searchResults = chunks.filter((chunk) => chunk.content?.toLowerCase().includes(query));
				break;
			case 'id':
				searchResults = chunks.filter((chunk) => chunk.id?.toString().includes(query));
				break;
			case 'level':
				...
				break;
			case 'parent':
				searchResults = chunks.filter((chunk) => chunk.parent_id?.toString().includes(query));
				break;
		}
	}
		...
		<div class="flex flex-col md:flex-row md:justify-between items-start gap-4 mb-6">
			<div class="flex-1">
				...
				{#if document.indexMessage}
					<div
						class={`mt-4 text-sm ${document.indexNeeded ? 'text-warning' : 'text-success'} flex items-center`}
					>
						<svg
			...
			<div class="flex flex-col gap-2">
				{#if !isEditing}
					<button class="btn btn-primary" onclick={startEditing} aria-label="Modifier le document">
						<svg
							xmlns="http://www.w3.org/2000/svg"
							class="h-5 w-5 mr-1"
							fill="none"
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
					<div class="stat p-2 bg-base-100 rounded-lg">
						...
						<div class="stat-value text-lg flex items-center">
							<span
								class={`w-3 h-3 inline-block rounded-full mr-2 ${indexStatus.indexExists ? 'bg-success' : 'bg-warning'}`}
							></span>
					...
					<div class="stat p-2 bg-base-100 rounded-lg">
						...
						<div class="stat-value text-lg">
							{indexStatus.indexedChunks} / {indexStatus.chunkCount}
						</div>
					</div>
		...
		<div class="flex flex-col gap-4 mt-8">
				...
				<div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
					<div class="form-control">
						...
						>
							...
							{#each getAvailableHierarchyLevels() as level (level)}
			...
			{:else}
				<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
					{#each getFilteredChunks() as chunk (chunk)}
						<button
							id={`chunk-${chunk.id}`}
				...
				<div class="text-sm text-gray-500 mt-2">
					{#if chunkSearchQuery.trim() || selectedLevel !== null}
						...
						{#if selectedLevel !== null}de niveau {selectedLevel}
						{/if}
						{#if chunkSearchQuery.trim()}correspondant à la recherche "{chunkSearchQuery}"
						{/if}
			...
			<div class="p-4">
				<div class="flex justify-between items-start mb-4">
					<div>
						...
						<div class="badge badge-outline ml-2 mt-1">
							Niveau {selectedChunk.hierarchyLevel ?? 0}
						</div>
					</div>
					<button
