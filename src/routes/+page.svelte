<script lang="ts">
	...
	async function handleRefresh() {
		...
		try {
			...
			error =
				err instanceof Error ? err.message : 'Une erreur est survenue lors du rafraîchissement';
		} finally {
			refreshing = false;
		}
	}
		...
		{:else if stats}
			<div class="stats shadow inline-flex flex-wrap">
				<div class="stat">
					...
					<div class="stat-desc">
						{stats.documentStats.percentChange > 0 ? '+' : ''}{stats.documentStats.percentChange}%
						par rapport à avant
					</div>
				</div>
				...
				<div class="stat">
					...
					<div class="stat-value text-secondary">
						{stats.searchStats.totalCount.toLocaleString('fr-FR')}
					</div>
					<div class="stat-desc">
				...
				<div class="stat">
					...
					<div class="stat-value text-info">
						{stats.systemStats.indexedCorpora}/{stats.systemStats.totalCorpora}
					</div>
					<div class="stat-desc">
						{Math.round((stats.systemStats.indexedCorpora / stats.systemStats.totalCorpora) * 100)}%
						de couverture
					</div>
				</div>
			</div>
			...
			{#if Object.keys(stats.documentStats.byTheme).length > 0}
				<div class="mt-8 max-w-xl mx-auto p-4 bg-base-200 rounded-lg">
					...
					<div class="flex gap-2 flex-wrap justify-center">
						{#each Object.entries(stats.documentStats.byTheme) as [theme, count] (theme)}
			...
			{#if stats.searchStats.topQueries && stats.searchStats.topQueries.length > 0}
				<div class="mt-4 max-w-xl mx-auto p-4 bg-base-200 rounded-lg">
					...
					<div class="flex gap-2 flex-wrap justify-center">
						{#each stats.searchStats.topQueries as { query, count } (query)}
